...
```
By involving the heteroskedasticity in the error term, the forecasting plot looks similar to the extended model, with the heteroskedasticity extension, it shows an increasing trend but reaching to the mean value more slowly and we could notice that the confident interval is narrower compared to the basic model, indicating a more informative estimating of the parameters as it has a higher certainty from a more concentrated mean value and then leading to a narrower confident interval as expected.
...
# Conclusion 
In this research project, we use one basic and three extended Bayesian models to forecast the AUD/USD exchange rate. For the basic model, we use Minnesota before fitting our non-stationary data property.  Then we applied the Laplace distribution to the error term to capture the leptokurtotic and anomies of the financial data. Then we introduced the stochastic volatility model to further capture the volatility of the error term.

We do the forecasting in h=12 steps which is a 3-year period in our case and for the point estimate of the forecasting, among four different models, the basic model forecasting shows a slightly decreasing trend, while the extended and its extension with heteroskedasticity has an opposite trend and shows a clearly increasing trend among forecasting periods.

And for the confident interval, compared to the basic model. The extended model and its extension provided a narrower confident interval which means a more informative and precise forecasting of the AUD/USD exchange rate.