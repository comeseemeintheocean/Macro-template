...
```{r}
Sigma_posterior_mean <- apply(posterior.sample.draws$Sigma.posterior, 1:2, base::mean)
...
A_posterior_means <- apply(posterior.sample.draws$A.posterior, 1:2, base::mean)
...
```{r}
Sigma_posterior_mean <- apply(posterior.extended$Sigma.posterior.exten, 1:2, base::mean)
...
```{r}
A_posterior_means <- apply(posterior.extended$A.posterior.exten, 1:2, base::mean)
...
point.f     = apply(Y.h[,1,],1,base::mean)
...
ex_rate.point.f    = apply(Y.h[,1,],1,base::mean) 
...
point.f         = apply(Y.h.ext[,1,],1,base::mean)
